...
import { fetchStuScoreList, fetchSemesterGradeList, fetchGradeList, deleteGrade, fetchExamListByClassId, fetchScoreListByClassAndSemester } from '@/api/user'
...
export default {
  ...
  data() {
    return {
      ...
      listQuery: {
        ...
        stuId: '0924d312-c57e-42b7-8765-0245b5a7a6d8',
        name: undefined,
  ...
  methods: {
    ...
    fetchStuScoreList() {
      if (this.listQuery.stuId) {
        fetchStuScoreList(this.listQuery.stuId).then(response => {
          this.semesterGradeList = response.data.result
        })
      }
    },
